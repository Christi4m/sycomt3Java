...
//metodo para listar el reporte de ventas por mes
    ...
    public ArrayList<Ventas> getAllEntregasPendientes() {

        ArrayList<Ventas> v = new ArrayList<>();
        try {
            String sql = "call entregasPendientes()";//crea la sentencia sql que voy a mandar a la base de datos
            pst = getConnection().prepareCall(sql);//abriendo la coneccion a la base de datos y los parametros que le voy a enviar (sentencia sql)
            rs = pst.executeQuery();//ejecutar la sentencia y trae un resultado
            while (rs.next()) {//ciclo repetitivo que llena el array list con los datos que trae la base de datos
                v.add(new Ventas(rs.getInt("idOrdenVenta"), rs.getInt("idTerceroF1"), rs.getInt("idFacturasVentas"),rs.getString("numFactura"), rs.getString("fechaOrdenVenta"), rs.getDouble("valorGlobal"), rs.getString("zona")));
            }
        } catch (Exception e) {
    ...
    public ArrayList<Ventas> getAllEntregasAsignadas() {
