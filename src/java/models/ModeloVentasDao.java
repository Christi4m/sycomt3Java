...
public class ModeloVentasDao extends Conexion {
    ...
    public ArrayList<Ventas> getAllVEntas() {
        ...
        try {
            ...
            rs = pst.executeQuery();//ejecutar la sentencia y trae un resultado
            while (rs.next()) {//ciclo repetitivo que llena el array list con los datos que trae la base de datos
                v.add(new Ventas(rs.getInt("idOrdenVenta"), rs.getString("fechaOrdenVenta"), rs.getDouble("valorGlobal"), rs.getInt("idTerceroF1"), rs.getString("numSerie"), rs.getString("estadoOrdenVenta"),rs.getInt("idFacturasVentas")));
            }
        } catch (Exception e) {
